...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card-header p { color: var(--gray-wood); font-size: 0.85rem; margin-top: 0.3rem; }

        .table-container { overflow-x: auto; padding: 0.5rem 0 1.5rem 0; }
        .table-container table { width: 100%; border-collapse: collapse; font-size: 0.8rem; }
        .table-container thead { background: var(--wood-dark); color: white; }
        .table-container th { padding: 0.6rem 0.8rem; text-align: center; font-weight: 600; font-size: 0.7rem; white-space: nowrap; }
        .table-container td { padding: 0.6rem 0.8rem; text-align: center; border-bottom: 1px solid rgba(0,0,0,0.05); color: var(--wood-dark); font-size: 0.75rem; vertical-align: middle; }
        .table-container tbody tr:hover { background: rgba(212,163,115,0.05); }

        .qty-editable { display: inline-flex; align-items: center; gap: 0.4rem; }
        .qty-editable .qty-value { font-size: 1rem; font-weight: 700; color: var(--wood-dark); min-width: 20px; }
        .qty-editable .qty-input { width: 55px; padding: 0.25rem 0.3rem; border: 2px solid var(--highlight-border); border-radius: 4px; font-size: 0.85rem; font-weight: 600; font-family: 'Inter', sans-serif; text-align: center; color: var(--wood-dark); outline: none; transition: all 0.3s; background: var(--highlight-bg); display: none; }
        .qty-editable .qty-input:focus { border-color: var(--accent-gold); box-shadow: 0 0 0 3px rgba(212,163,115,0.15); }
        .qty-editable .qty-input.show { display: inline-block; }
        .qty-editable .qty-value.hide { display: none; }
        .qty-editable .edit-icon { color: var(--gray-wood); cursor: pointer; font-size: 0.8rem; transition: all 0.3s; padding: 2px 4px; border-radius: 4px; }
        .qty-editable .edit-icon:hover { color: var(--accent-gold); background: rgba(212,163,115,0.1); }
        .qty-editable .save-icon { color: #2d6a4f; cursor: pointer; font-size: 0.8rem; transition: all 0.3s; padding: 2px 4px; border-radius: 4px; display: none; }
        .qty-editable .save-icon.show { display: inline-block; }
        .qty-editable .save-icon:hover { background: rgba(45,106,79,0.1); transform: scale(1.1); }

        .status-badge { padding: 3px 10px; border-radius: 12px; font-size: 0.7rem; font-weight: 600; display: inline-block; }
        ...
        .action-form { display: flex; gap: 0.3rem; align-items: center; justify-content: center; margin: 0; flex-wrap: wrap; }
        .status-select { padding: 0.3rem 0.5rem; border-radius: 6px; border: 1.5px solid var(--input-border); font-size: 0.7rem; font-family: 'Inter', sans-serif; background: white; color: var(--wood-dark); outline: none; transition: all 0.3s; min-width: 90px; }
        .status-select:focus { border-color: var(--accent-gold); box-shadow: 0 0 0 3px rgba(212,163,115,0.15); }
        .btn-update { padding: 0.3rem 0.8rem; font-size: 0.7rem; background: var(--wood-medium); color: white; border: none; border-radius: 6px; cursor: pointer; transition: all 0.3s; font-family: 'Inter', sans-serif; font-weight: 600; white-space: nowrap; }
        .btn-update:hover { background: var(--wood-dark); transform: translateY(-2px); box-shadow: 0 4px 15px rgba(62,42,33,0.3); }
        .btn-update i { margin-right: 3px; }

        .material-info { font-size: 0.7rem; line-height: 1.4; max-width: 150px; }
        .material-info .material-item { display: block; padding: 1px 0; }
        .material-info .material-name { font-weight: 600; color: var(--wood-dark); }
        .material-info .material-stock { color: var(--gray-wood); }

        .alert-container { padding: 1.5rem 2rem 1rem 2rem; display: flex; justify-content: center; }
        .alert { padding: 0.8rem 2rem; border-radius: 0.8rem; font-size: 0.9rem; text-align: center; font-weight: 500; display: inline-block; min-width: 300px; }
        ...
        @media (max-width: 1024px) { .table-container table { min-width: 1200px; } }
        @media (max-width: 768px) {
            ...
            .alert-container { padding: 1rem 1rem 0.5rem 1rem; }
            .alert { min-width: unset; width: 100%; padding: 0.8rem 1rem; }
            .action-form { flex-direction: column; gap: 0.3rem; }
            .status-select { width: 100%; min-width: unset; }
            .btn-update { width: 100%; }
            .qty-editable .qty-input { width: 100%; }
        }
...
<div class="container">
    <div class="card">
        <div class="card-header">
            ...
            <p>Manage order statuses and quantities</p>
        </div>

        <?php if (!empty($msg)): ?>
            <div class="alert-container"><?php echo $msg; ?></div>
        <?php endif; ?>
        ...
        <div class="table-container">
            <table>
                ...
                <tr>
                    ...
                    <th>Product</th>
                    <th>Image</th>
                    <th style="background: #4caf50; color: white;">📦 Qty</th>
                    <th>Total</th>
                ...
                <tbody>
                <?php if (empty($orders)): ?>
                    <tr><td colspan="12"><div class="empty-state"><i class="fas fa-inbox"></i><h3>No Orders Found</h3><p>No customer orders found in the system.</p></div></td></tr>
                <?php else: ?>
                    <?php foreach ($orders as $row):
                        ...
                        <tr>
                            ...
                            <td style="background: #f1f8e9;">
                                <form action="manage-orders.php" method="POST" style="display:inline;">
                                    <input type="hidden" name="oid" value="<?php echo $row['oid']; ?>">
                                    <input type="hidden" name="fid" value="<?php echo $row['fid'] ?? 0; ?>">
                                    <input type="hidden" name="new_qty" id="hidden_qty_<?php echo $row['oid']; ?>" value="<?php echo $row['oqty'] ?? 1; ?>">
                                    <input type="number" name="new_qty_display" class="qty-input show" style="display:inline-block; width:60px; padding:0.3rem; border:2px solid #4caf50; border-radius:4px; text-align:center; font-weight:600;"
                                           value="<?php echo $row['oqty'] ?? 1; ?>" min="1"
                                           onchange="document.getElementById('hidden_qty_<?php echo $row['oid']; ?>').value=this.value">
                                </form>
                            </td>
                            <td><strong>$<?php echo number_format($row['ototalamount'] ?? 0, 2); ?></strong></td>
                            <td style="max-width: 120px; font-size: 0.7rem;"><?php echo htmlspecialchars(substr($row['odeliveraddress'] ?? '', 0, 20)) . (strlen($row['odeliveraddress'] ?? '') > 20 ? '...' : ''); ?></td>
                            ...
                            <td><span class="status-badge <?php echo $badge_class; ?>"><?php echo ucfirst($row['status'] ?? 'Open'); ?></span></td>
                            <td>
                                <form action="manage-orders.php" method="POST" class="action-form">
                                    <input type="hidden" name="oid" value="<?php echo $row['oid']; ?>">
                                    <input type="hidden" name="fid" value="<?php echo $row['fid'] ?? 0; ?>">
                                    <input type="hidden" name="new_qty" id="form_qty_<?php echo $row['oid']; ?>" value="<?php echo $row['oqty'] ?? 1; ?>">
...
<script>
    // Sync quantity from display input to hidden form input
    document.querySelectorAll('.qty-input').forEach(input => {
        input.addEventListener('change', function() {
            const oid = this.closest('td').querySelector('input[name="oid"]').value;
            const hidden = document.getElementById('hidden_qty_' + oid);
            const formHidden = document.getElementById('form_qty_' + oid);
            if (hidden) hidden.value = this.value;
            if (formHidden) formHidden.value = this.value;
        });
    });
</script>
</body>
