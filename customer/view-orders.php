...
try {
    $query = "SELECT o.oid, o.cid, o.status, o.odate, o.odeliverydate, of.fid, of.oqty, f.fname, f.fprice, o.ototalamount
              ...
              WHERE o.cid = ? 
              ORDER BY $sort_by $sort_order, o.oid DESC";
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sort-controls label { font-weight: 600; color: var(--wood-dark); font-size: 0.85rem; }
        .sort-controls label i { color: var(--accent-gold); margin-right: 0.3rem; }
        .sort-controls select { padding: 0.4rem 0.8rem; border: 1.5px solid var(--input-border); border-radius: 0.5rem; font-family: 'Inter', sans-serif; font-size: 0.85rem; color: var(--wood-dark); background: white; outline: none; cursor: pointer; transition: all 0.3s; min-width: 200px; }
        .sort-controls select:focus { border-color: var(--accent-gold); box-shadow: 0 0 0 3px rgba(212,163,115,0.15); }
        .sort-controls .sort-indicator { font-size: 0.8rem; color: var(--gray-wood); }

        .table-container { overflow-x: auto; padding: 0; }
        .table-container table { width: 100%; border-collapse: collapse; }
        .table-container thead { background: var(--wood-dark); color: white; }
        .table-container th { padding: 0.8rem 1rem; text-align: center; font-weight: 600; font-size: 0.75rem; white-space: nowrap; }
        .table-container th a { color: white; text-decoration: none; display: inline-flex; align-items: center; gap: 0.3rem; }
        .table-container th a:hover { color: var(--accent-gold); }
        .table-container td { padding: 0.8rem 1rem; text-align: center; border-bottom: 1px solid rgba(0,0,0,0.05); color: var(--wood-dark); font-size: 0.8rem; vertical-align: middle; }
        .table-container tbody tr:hover { background: rgba(212,163,115,0.05); }

        .badge { display: inline-block; padding: 0.25rem 1rem; border-radius: 2rem; font-size: 0.7rem; font-weight: 600; }
        ...
        .btn { display: inline-block; padding: 0.3rem 0.8rem; border: none; border-radius: var(--radius-btn); cursor: pointer; font-weight: 600; text-decoration: none; transition: all 0.3s ease; font-size: 0.75rem; text-align: center; font-family: 'Inter', sans-serif; }
        ...
        @media (max-width: 1024px) { .table-container table { min-width: 900px; } }
        @media (max-width: 768px) {
            ...
            .sort-controls { flex-direction: column; align-items: stretch; padding: 1rem; gap: 0.5rem; }
            .sort-controls select { width: 100%; min-width: unset; }
        }
...
<div class="container">
    <div class="card">
        ...
        <div class="sort-controls">
            <div><i class="fas fa-sort"></i> <label for="sortSelect">Sort by:</label></div>
            ...
            <span class="sort-indicator">
                <i class="fas fa-arrow-<?php echo $sort_order === 'DESC' ? 'down' : 'up'; ?>"></i>
                Sorting by: <strong><?php echo ucfirst($sort_by); ?></strong> (<?php echo $sort_order; ?>)
            </span>
        </div>
        ...
        <div class="table-container">
            <table>
                <thead>
                <tr>
                    <th><a href="?sort=oid&order=<?php echo ($sort_by === 'oid' && $sort_order === 'DESC') ? 'ASC' : 'DESC'; ?>">Order ID <i class="fas fa-sort"></i></a></th>
                    <th><a href="?sort=odate&order=<?php echo ($sort_by === 'odate' && $sort_order === 'DESC') ? 'ASC' : 'DESC'; ?>">Date <i class="fas fa-sort"></i></a></th>
                    <th><a href="?sort=odeliverydate&order=<?php echo ($sort_by === 'odeliverydate' && $sort_order === 'DESC') ? 'ASC' : 'DESC'; ?>">Delivery <i class="fas fa-sort"></i></a></th>
                    <th><a href="?sort=fname&order=<?php echo ($sort_by === 'fname' && $sort_order === 'DESC') ? 'ASC' : 'DESC'; ?>">Product <i class="fas fa-sort"></i></a></th>
                    <th>Furniture ID</th>
                    <th>Quantity</th>
                    <th><a href="?sort=ototalamount&order=<?php echo ($sort_by === 'ototalamount' && $sort_order === 'DESC') ? 'ASC' : 'DESC'; ?>">Total <i class="fas fa-sort"></i></a></th>
                    <th><a href="?sort=status&order=<?php echo ($sort_by === 'status' && $sort_order === 'DESC') ? 'ASC' : 'DESC'; ?>">Status <i class="fas fa-sort"></i></a></th>
                    <th>Action</th>
                </tr>
                </thead>
                <tbody>
                <?php if (empty($orders)): ?>
                    <tr><td colspan="9"><div class="empty-state"><i class="fas fa-inbox"></i><h3>No Orders Yet</h3><p>You haven't placed any orders yet.</p><a href="make-order.php" class="btn btn-primary" style="margin-top:1rem;display:inline-block;padding:0.6rem 1.5rem;font-size:0.9rem;"><i class="fas fa-shopping-cart"></i> Start Shopping</a></div></td></tr>
                <?php else: ?>
                    <?php foreach ($orders as $order):
                        ...
                        <tr>
                            ...
                            <td><span class="badge <?php echo $badge_class; ?>"><?php echo htmlspecialchars($order['status']); ?></span></td>
                            <td><a href="view-order-details.php?oid=<?php echo $order['oid']; ?>" class="btn btn-primary"><i class="fas fa-eye"></i></a></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
                </tbody>
