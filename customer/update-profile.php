...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validation
    if (empty($full_name) || empty($phone) || empty($email) || empty($delivery_address)) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("UPDATE users SET full_name = ?, phone = ?, email = ?, delivery_address = ? WHERE id = ?");
            $stmt->bind_param("ssssi", $full_name, $phone, $email, $delivery_address, $user_id);
...
// Read current fields from Database row - Updated column names
$stmt = $conn->prepare("SELECT full_name, phone, email, delivery_address FROM users WHERE id = ?");
